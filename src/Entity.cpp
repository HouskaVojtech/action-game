...
Vector2 Entity::get_width_height () const
{
...
bool Entity::intersects ( Entity * e ) const
{
  if (
...
bool Entity::intersects ( int x, int y, int height, int width ) const
{
  if (
...
void Projectile::on_event ( SDL_Event & event ) {
  if ( event . type == SDL_USEREVENT ) {
    if ( event . user . code == TIME_EVENT_CODE ) {
...
void Player::set_colliders ( std::list<std::unique_ptr<Entity>> *walls, std::list<std::unique_ptr<Entity>> * projectiles )
{
  this->walls = walls;
...
void Player::on_event ( SDL_Event & event) 
{
      ...
      switch ( event . key . keysym . sym ) 
      {
          ...
          case SDLK_SPACE:
              if ( can_shoot ) {
                ...
                // creates projectile and passes it to its container
                std::unique_ptr <Projectile> tmp = std::make_unique<Projectile> (start_x, start_y, PROJECTILE_WIDTH, PROJECTILE_HEIGHT, "src/data/projectile.png", angle );
  ...
  if ( event . type == SDL_USEREVENT ) {
    if ( event . user . code == TIME_EVENT_CODE ) {
      ...
      // checks whether player is out of map
      ...
      // sends player position on player's move
      player_position_event . user . code = PLAYER_MOVE_EVENT_CODE;
...
void Enemy::set_colliders ( std::list<std::unique_ptr<Entity>> * walls )
{
  this -> walls = walls;
}
...
void Enemy::on_event ( SDL_Event & event ) {
  ...
  if ( event . type == SDL_USEREVENT )
  {
    if ( event . user .  code == TIME_EVENT_CODE )
    {
      // animate the sprite and use position . x as random seed
...
void Banner::on_event ( SDL_Event & ) {}

void Banner::delay ( uint32_t ms ) const
{
  SDL_Delay ( ms );
}