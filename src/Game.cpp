...
class unset_enemies : public std::exception
{
  virtual const char * what() const throw()
  {
    return "No enemies has been set" ;
  }
};
...
Game::Game ( const char * name,
             bool fullscreen,
             const char * map_path  ) 
            : player ( 0, 0, UNIT, UNIT, "src/data/player.png", 0, 0 )
{
  // initialize SDL 0 return value means no errors -1 means errors
  if ( SDL_Init ( SDL_INIT_EVERYTHING ) == 0 )
  {
    ...
    banner . init ( (map_width - PREPARE_WIDTH )/2 , (map_height - PREPARE_HEIGHT) / 2, PREPARE_WIDTH, PREPARE_HEIGHT, "src/data/prepare.png"  );
...
// function to set probability of bonuses dorps 
bool coin_flip ( unsigned short prob ) {
  std::random_device r;
  std::mt19937 random_engine (r());
  // uses uniform distribution form 1 to 100 so probability is easy to set to whole percents
...
void Game::collisions() {
  ...
  for ( auto p_it = projectiles . begin (); p_it != projectiles . end(); ) {
    bool deleted = false;
      for ( auto e_it = enemies . begin (); e_it != enemies . end (); )
      {
        ...
        {
          p_it = projectiles . erase ( p_it );
          if ( coin_flip ( BONUS_PROBABILITY ) ) {
            Vector2 v = ( *e_it ) -> get_position();
            std::cout << "bonus granted" << std::endl;

            // pushes new bonus to its container
            std::unique_ptr<Bonus> tmp =std::make_unique<Bonus> ( v . x , v . y , UNIT,UNIT, "src/data/bonus.png" ); 
      ...
      if ( enemies . size () == 0 )
      {
        banner . init ( (map_width - WIN_WIDTH )/2 , (map_height - WIN_HEIGHT) / 2, WIN_WIDTH, WIN_HEIGHT, "src/data/win.png" );
        banner . set_renderer ( renderer );
        banner . load_texture ();
        banner_render ( banner );
  ...
  for ( auto & e : enemies )
  {
    if ( e -> intersects ( &player ) )
    {
      ...
      banner . init ( (map_width - GAME_OVER_WIDTH )/2 , (map_height - GAME_OVER_HEIGHT) / 2, GAME_OVER_WIDTH, GAME_OVER_HEIGHT, "src/data/game_over.png" );
      banner . set_renderer ( renderer );
      banner . load_texture ();
      banner_render ( banner );
  ...
  for ( auto b_it = bonuses . begin(); b_it != bonuses . end(); ) {
      if ( player . intersects ( b_it -> get () ) )
      {
        ...
        if ( coin_flip ( RELOAD_BONUS_PROBABILITY ) )
        {
...
void Game::handle_events()
{
	...
	while ( SDL_PollEvent ( &event ) )
  {
    ...
    {
      if ( event . user . code == PLAYER_MOVE_EVENT_CODE )
      {
        // deletes alocated data that were used to send message to entities
    ...
    while ( map_file . get ( c ) ) {
      switch ( c ){
        ...
        case 'E':
          ...
          {
            std::cout << c;
            std::unique_ptr<Enemy> tmp = std::make_unique<Enemy> ( x_index * UNIT,y_index * UNIT,UNIT,UNIT, "src/data/enemy.png" );
        ...
        case '#':
          // add wall entity
          std::cout << c;
          walls . emplace_back ( std::make_unique<Wall> ( x_index * UNIT,y_index * UNIT,UNIT,UNIT, "src/data/wall.png" ) );
          ++x_index;
          break;
